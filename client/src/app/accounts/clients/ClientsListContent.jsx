...
import { getClients, deleteClient } from "@/actions/clients.action";
...
export default function ClientsListContent({ initialData }) {
  ...
  const handleSearch = (e) => {
    ...
    setSearch(val);
    setPage(1);
    fetchClients({ search: val, page: 1 });
  };
  ...
  const handleFilter = (e) => {
    ...
    setStatus(val);
    setPage(1);
    fetchClients({ status: val, page: 1 });
  };
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center justify-end gap-1">
          ...
          <Link
            href={`/accounts/clients/${row.id}/edit`}
          ...
          {["INACTIVE", "CHURNED"].includes(row.status) && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                setDeleteModal({ open: true, client: row });
              }}
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <PageHeader
        title="Clients"
        description="Manage your active clients and company relationships."
        breadcrumbs={[
      ...
      <div className="bg-white dark:bg-slate-950 rounded-[24px] p-6 border border-slate-100 dark:border-slate-800 shadow-sm dark:shadow-none shadow-slate-200/50 dark:shadow-none">
        <DataTable
          ...
          onRowClick={(row) => router.push(`/accounts/clients/${row.id}`)}
      ...
      <ConfirmModal
        ...
        title="Delete Client"
        message={`Are you sure you want to delete "${deleteModal.client?.companyName}"? This action cannot be undone.`}

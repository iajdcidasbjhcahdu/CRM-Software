...
export default function AccountDocumentsContent({ initialData }) {
  ...
  useEffect(() => { const t = setTimeout(fetchDocuments, 300); return () => clearTimeout(t); }, [fetchDocuments]);
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Documents from your managed projects.</p>
      </div>
          ...
          {documents.map((doc) => (
            <div key={doc.id} className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center flex-shrink-0"><FileText className="w-5 h-5 text-indigo-600" /></div>

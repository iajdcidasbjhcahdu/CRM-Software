        ...
        {Icon && (
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center ${
              accent ? "bg-white dark:bg-slate-950/20" : "bg-slate-50 dark:bg-slate-900 text-slate-400"
            }`}
          >
...
function AvatarBadge({ initials, name, label }) {
  return (
    <div className="flex items-center gap-3 py-3 border-b border-slate-50 dark:border-slate-800 last:border-0">
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: "Projects", href: "/accounts/projects" },
          { label: project.name },
        ]}
        actions={
          <Link
            href={`/accounts/projects/${project.id}/edit`}
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="flex-1 pt-4">
              <div className="flex flex-col md:flex-row md:items-center gap-3 mb-2">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-50">{project.name}</h2>
                <Badge value={project.status} />
              </div>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                Client:{" "}
                <Link
                  href={`/accounts/clients/${project.client?.id}`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-950 rounded-[24px] p-6 lg:p-8 border border-slate-100 dark:border-slate-800 shadow-sm dark:shadow-none shadow-slate-200/50 dark:shadow-none">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-50">Client Details</h3>
          </div>
          <div className="flex flex-col">
            <InfoRow
              icon={Building2}
              ...
              link={`/accounts/clients/${project.client.id}`}
            />
            <InfoRow
              icon={User}
              label="Contact"
            ...
            />
            <div className="flex items-center gap-3 py-2 border-b border-slate-50 dark:border-slate-800">
              <AlertCircle className="w-4 h-4 text-slate-400 shrink-0" />
              <span className="text-sm text-slate-500 dark:text-slate-400 min-w-[100px]">Status</span>
            ...
            {project.deal && (
              <>
                <InfoRow
                  ...
                  link={`/accounts/deals/${project.deal.id}`}
                />
                <InfoRow
                  icon={DollarSign}
                  label="Deal Value"
                ...
                <div key={ps.id} className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <Link href={`/accounts/services/${ps.service?.id}`} className="text-sm font-semibold text-slate-900 dark:text-slate-50 hover:text-indigo-600 transition-colors">
                        {ps.service?.name}
                      </Link>

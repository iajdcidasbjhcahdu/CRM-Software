...
const TEMPLATE_COLORS = {
  "login-otp": { bg: "bg-indigo-50", text: "text-indigo-600" },
  "welcome-user": { bg: "bg-emerald-50", text: "text-emerald-600" },
  "reset-password": { bg: "bg-amber-50", text: "text-amber-600" },
...
export default function EmailTemplatesTab() {
  ...
  if (selectedTemplate) {
    ...
    const colors = TEMPLATE_COLORS[selectedTemplate.slug] || { bg: "bg-slate-50", text: "text-slate-600" };
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={handleBack}
            className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-slate-50 hover:text-slate-900 transition-colors shadow-sm"
          >
        ...
        {isEmpty && (
          <div className="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium bg-amber-50 text-amber-700 border border-amber-200">
        ...
        {variables.length > 0 && (
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
            <p className="text-xs font-medium text-slate-500 mb-2">Available Variables (use in subject & body)</p>
            <div className="flex flex-wrap gap-2">
              {variables.map((v) => (
                <span
                  key={v}
                  className="px-2.5 py-1 text-xs font-mono font-medium text-indigo-600 bg-indigo-50 border border-indigo-100 rounded-lg cursor-pointer hover:bg-indigo-100 transition-colors"
        ...
        <SettingsCard title="Email Subject">
          <input
            ...
            className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
        ...
        {viewMode === "edit" ? (
          <SettingsCard title="Email Body (HTML)">
            <textarea
              ...
              rows={20}
              className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm text-slate-700 font-mono leading-relaxed focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all resize-y"
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {templates.map((template) => {
            ...
            const colors = TEMPLATE_COLORS[template.slug] || { bg: "bg-slate-50", text: "text-slate-600" };
            ...
            return (
              <button
                ...
                className="flex items-center gap-4 p-4 rounded-2xl border border-slate-200 bg-white hover:border-indigo-200 hover:shadow-md transition-all text-left group"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isEmpty && (
                      <span className="px-2 py-0.5 text-[10px] font-bold bg-amber-50 text-amber-600 border border-amber-200 rounded-full shrink-0">
                        EMPTY
                      </span>
                    )}
                    {!isEmpty && (
                      <span className="px-2 py-0.5 text-[10px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-full shrink-0">
                        ACTIVE
                      </span>
                    )}
                  </div>

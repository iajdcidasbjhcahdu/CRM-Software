          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="flex-1 pt-4">
              ...
              <p className="text-sm text-slate-500 mb-4">
                Contact: <span className="font-medium text-slate-700">{client.contactName}</span>
              </p>
          ...
          <div className="flex flex-col gap-3">
            {projects.map((project) => (
              <Link
                ...
                className="flex flex-col lg:flex-row lg:items-center gap-3 p-4 rounded-xl bg-slate-50 border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/30 transition-all"
              >
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <span className={`font-medium text-sm ${accent ? "text-indigo-200" : "text-slate-600"}`}>
          {label}
        </span>
        {Icon && (
          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${accent ? "bg-white/20" : "bg-slate-50 text-slate-400"}`}>
            <Icon className="w-4 h-4" />
          </div>
        )}
      </div>
      <div>
        <span className={`text-lg font-bold ${accent ? "text-white" : "text-slate-900"}`} suppressHydrationWarning>
          {value}
        </span>
        {subtext && (

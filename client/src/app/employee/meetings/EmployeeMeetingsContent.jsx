...
export default function EmployeeMeetingsContent({ initialData }) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Meetings from your assigned projects.</p>
      </div>
        ...
        <div className="space-y-3">
          {meetings.map((meeting) => {
            ...
            return (
              <div
                key={meeting.id}
                className={`bg-white dark:bg-slate-950 rounded-2xl border p-5 transition-colors ${
                  upcoming ? "border-[#5542F6]/20 dark:border-[#5542F6]/30" : "border-slate-200 dark:border-slate-800"
                }`}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                      upcoming ? "bg-[#5542F6]/10" : "bg-slate-100 dark:bg-slate-800"
                    }`}>
                    ...
                    <div className="min-w-0">
                      ...
                      {meeting.description && (
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 line-clamp-2">{meeting.description}</p>
                      )}
                    </div>
                  </div>
                ...
                {meeting.link && upcoming && (
                  <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                    <a
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 bg-[#5542F6] text-white text-sm font-medium rounded-xl hover:bg-[#4636d4] transition-colors"
                    >
                      <ModeIcon className="w-4 h-4" />
                      {meeting.mode === "VIRTUAL" ? "Join Meeting" : meeting.mode === "PHONE_CALL" ? "Call" : "Get Directions"}
                    </a>
                  </div>
                )}
              </div>

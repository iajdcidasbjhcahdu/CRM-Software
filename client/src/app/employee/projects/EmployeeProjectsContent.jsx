...
import { FolderKanban, Search, Calendar, User, Loader2, Users } from "lucide-react";
...
export default function EmployeeProjectsContent({ initialData }) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">My Projects</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Projects you are assigned to through your team.</p>
      </div>
      ...
      {loading ? (
              ...
              <div className="space-y-2 text-xs text-slate-500 dark:text-slate-400">
                ...
                {project.projectTeams?.length > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <span>{project.projectTeams.length} team{project.projectTeams.length !== 1 ? "s" : ""}</span>
                  </div>
                )}
        ...
        <div className="bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 p-12 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400">No projects assigned to you yet.</p>
        </div>
      )}
    </div>
  );
